...
public class LinkedList<AnyType extends Comparable<? super AnyType>> implements List<AnyType> {
    ...
    public void addInOrder(AnyType t) throws NullPointerException {
    	...
    	Node<AnyType> tmpNode = new Node<>(t);
    	
    	System.out.println("head node : " + headNode);
    	
    	if (isEmpty() || walker == null) {
    		headNode = tmpNode;
    	} else {
        	int where = 0;        	
        	while (where < size && t.compareTo(walker.getData()) >= 0) {        		
        		walker = walker.getNextNode();
        		where++;
        	} 
        	
        	addNode(where, tmpNode);
    	}
    ...
    public static void testAddInOrder(LinkedList<Integer> list) {
        try {
        	list.addInOrder(19);
